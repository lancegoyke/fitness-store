...
import type { Day, Exercise, GridDay, GridWeek, MesoGrid, Phase } from "./api";
...
/** Whether this program day has a live session in the given week. */
export function cellOn(day: Pick<GridDay, "session_ids">, week: Pick<GridWeek, "id">): boolean {
  return day.session_ids[String(week.id)] != null;
}
...
export function cellStyle(
  day: Pick<GridDay, "session_ids">,
  week: Pick<GridWeek, "id">,
): string {
  const on = cellOn(day, week);
 ...
 */
export function cycleLabelFromGrid(phases: Phase[], weeks: Pick<GridWeek, "id" | "label">[]): string {
