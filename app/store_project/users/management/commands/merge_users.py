...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Stripe Customer ID: '{source_user.stripe_customer_id}' → '{target_user.stripe_customer_id}'")
        ...
        try:
            with transaction.atomic():
                ...
                if records_count > 0:
                    ...
                    self.stdout.write(f"✓ Transferred {records_count} challenge records")

                # Transfer programs and books
                if programs_count > 0:
                    Program.objects.filter(author=source_user).update(author=target_user)
                ...
                if target_user.sex == User.Sex.UNKNOWN and source_user.sex != User.Sex.UNKNOWN:
                    target_user.sex = source_user.sex
                if not target_user.stripe_customer_id and source_user.stripe_customer_id:
                    target_user.stripe_customer_id = source_user.stripe_customer_id

                target_user.save()
                self.stdout.write(f"✓ Updated target user data")
