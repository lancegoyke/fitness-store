...
``settle.settle_log`` takes the SAME Plan-first, Session-self-only lock pair as
both athlete write paths. It takes both before re-reading the log, so a
concurrent blur/save and the sweep can never both act on stale state (see
``settle.py``'s module docstring for the full contract).
...
def test_settle_log_blocks_on_a_concurrently_held_session_lock(client):
    ...
    def hold_then_bump_and_commit():
        ...
        # lock pair those paths take, then (while still holding it) bumps
        ...
        try:
            with transaction.atomic():
                Plan.objects.select_for_update(no_key=True).filter(pk=s.plan.pk).first()
